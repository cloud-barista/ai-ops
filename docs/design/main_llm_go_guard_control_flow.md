# LLM Deployment Planner와 Go Guard 연계 흐름

## 1. 역할

이 저장소의 핵심 역할은 **AppDeploy 앞단의 LLM Deployment Planner**입니다. 사용자의 자연어 배포 요구를 분석해 CPU, 메모리, GPU, 디스크, accelerator 요구량을 결정하고 AppDeploy 공식 `DeploymentManifest`를 생성합니다.
...
flowchart LR
    U["자연어 배포 요구"]
    P["LLM Deployment Planner<br/>자원 요구량 결정"]
    M["DeploymentManifest"]
    G["Go Guard<br/>형식·값·보안 검증"]
...
Go Guard를 통과한 Manifest는 AppDeploy의 다음 공식 API에 전달됩니다.
...
| Go Guard | Manifest 계약과 정책의 결정적 검증 |
