...
| Local API 기본 응답 확인 | API 서버 실행 후 주요 endpoint 호출 | endpoint availability와 JSON response structure 확인 |
| Local API integration validation | `go run ./cmd/aiops-service-control api-integration-validation --output-dir ../../runs/api-integration-local --port 18080` | Go CLI 기반 6개 endpoint 순차 호출과 핵심 response field 검증 |
...
대표 로컬 검증 결과는 [`local_validation_20260703.md`](local_validation_20260703.md)에 정리되어 있습니다. 이 문서는 Go test, `team-validation`, `validate-system --target local`, 로컬 API 통합 검증, Ops LLM dry-run/evaluator 결과를 제출용 요약과 대표 JSON 산출물로 연결합니다.

로컬 API 기본 응답 확인은 production-level operational validation이 아닙니다. Local API integration validation도 로컬 service-control API flow의 필드 검증이며, 실제 운영 배포나 cloud provisioning 완료를 의미하지 않습니다.
...
대표 VM 검증 결과는 [`vm_validation_20260707.md`](vm_validation_20260707.md)에 정리되어 있습니다. 원본 `runs/full-validation-vm-gpu/` 전체가 아니라, 제출용으로 필요한 summary와 대표 artifact만 `docs/evidence/artifacts/`에 보존합니다.

실제 LLM endpoint 및 Kubernetes live path 검증 결과는 [`local_llm_k8s_validation_20260707.md`](local_llm_k8s_validation_20260707.md)에 정리되어 있습니다. 이 문서는 `benchmark_status=executed` LLM 평가, API integration, Kubernetes server dry-run, 검증용 live deployment, `aiops-guard` real mode scale 증적을 분리해 기록합니다.
