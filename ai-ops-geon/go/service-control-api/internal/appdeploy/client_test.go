...
func TestClientDecodesRetryableAppDeployError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		...
		_, _ = writer.Write([]byte(`{
			...
			"error":{"code":"AI_INFRA_API_TIMEOUT","message":"temporary failure","details":{"deployment_id":"dep-error","status":"DEPLOYMENT_FAILED"},"retryable":true}
		}`))
	}))
	defer server.Close()
	...
	if !apiError.Retryable || apiError.Code != "AI_INFRA_API_TIMEOUT" || apiError.RequestID != "req-error" || apiError.Details["deployment_id"] != "dep-error" {
