...
func (service *Service) ReceiveDeploymentStatus(
	...
	flow.RepairDecision = buildRepairDecision(flow, service.now())
	flow.UpdatedAt = service.now().Format(time.RFC3339Nano)
	service.flows[message.CorrelationID] = cloneFlow(flow)
	return cloneFlow(flow), nil
}
...
func (service *Service) evaluateFlow(ctx context.Context, flow Flow) Flow {
	...
	if issues := validateApplicationRequirements(profile.Requirements); len(issues) > 0 {
		...
		flow.Decision = &AutomationDecision{
			...
			ReasonCodes:                []string{ReasonApplicationRequirementsInvalid},
	...
	if len(issues) > 0 {
		...
		flow.Decision = &AutomationDecision{
			...
			ReasonCodes:                []string{ReasonResourceRecommendationInvalid},
			ReasoningMode:              ReasoningModeRuleBased,
			Confidence:                 1,
			RequiresManifestGeneration: false,
			RetryPolicy:                &RetryPolicy{MaxAttempts: 2, Attempt: 1},
			CorrectionRequest: &CorrectionRequest{
	...
	flow.Decision = &AutomationDecision{
		...
		ReasonCodes:                []string{ReasonRequirementsSatisfied, ReasonResourceRecommendationValid},
		ReasoningMode:              ReasoningModeRuleBased,
		Confidence:                 candidate.Scores.Total,
		SelectedCandidateID:        candidate.CandidateID,
		RequiresManifestGeneration: true,
...
func validateApplicationRequirements(requirements ApplicationRequirements) []ValidationIssue {
	...
	if requirements.Accelerator.CountMin < 0 {
		issues = append(issues, ValidationIssue{
			Field:    "requirements.accelerator.count_min",
			Code:     "VALUE_MUST_NOT_BE_NEGATIVE",
			Reason:   "Accelerator count must be zero or greater.",
			Expected: ">= 0",
			Actual:   requirements.Accelerator.CountMin,
		})
	}
	if requirements.Accelerator.MemoryMiBMinPerDevice < 0 {
		issues = append(issues, ValidationIssue{
			Field:    "requirements.accelerator.memory_mib_min_per_device",
			Code:     "VALUE_MUST_NOT_BE_NEGATIVE",
			Reason:   "Accelerator memory must be zero or greater.",
			Expected: ">= 0",
			Actual:   requirements.Accelerator.MemoryMiBMinPerDevice,
		})
	}
	if requirements.SLO.LatencyP95MSMax < 0 {
		issues = append(issues, ValidationIssue{
			Field:    "requirements.slo.latency_p95_ms_max",
			Code:     "VALUE_MUST_NOT_BE_NEGATIVE",
			Reason:   "Maximum latency must be zero or greater.",
			Expected: ">= 0",
			Actual:   requirements.SLO.LatencyP95MSMax,
		})
	}
	if requirements.SLO.ThroughputRPSMin < 0 {
		issues = append(issues, ValidationIssue{
			Field:    "requirements.slo.throughput_rps_min",
			Code:     "VALUE_MUST_NOT_BE_NEGATIVE",
			Reason:   "Minimum throughput must be zero or greater.",
			Expected: ">= 0",
			Actual:   requirements.SLO.ThroughputRPSMin,
		})
	}
	if requirements.Cost.CostPerHourMax < 0 {
		issues = append(issues, ValidationIssue{
			Field:    "requirements.cost.cost_per_hour_max",
			Code:     "VALUE_MUST_NOT_BE_NEGATIVE",
			Reason:   "Maximum hourly cost must be zero or greater.",
			Expected: ">= 0",
			Actual:   requirements.Cost.CostPerHourMax,
		})
	}
	return issues
}
...
func selectRecommendedCandidate(
	...
	if recommendation.Status != "FOUND" {
		return ResourceCandidate{}, []ValidationIssue{{
			Field:    "resource_recommendation.status",
			Code:     "RECOMMENDATION_NOT_READY",
			Reason:   "The resource recommendation is not ready for deployment.",
			Expected: "FOUND",
			Actual:   recommendation.Status,
		}}
	}
...
func validateResourceRecommendation(recommendation ResourceRecommendation) error {
	...
	case recommendation.Status != "FOUND" && recommendation.Status != "RETRY_REQUIRED":
		return fmt.Errorf("resource_recommendation.status is not supported: %s", recommendation.Status)
	}
	seenCandidateIDs := make(map[string]struct{}, len(recommendation.Candidates))
	for _, candidate := range recommendation.Candidates {
		candidateID := strings.TrimSpace(candidate.CandidateID)
		if candidateID == "" {
			return fmt.Errorf("resource_recommendation.candidate_id is required")
		}
		if _, exists := seenCandidateIDs[candidateID]; exists {
			return fmt.Errorf("resource_recommendation.candidate_id must be unique: %s", candidateID)
		}
		seenCandidateIDs[candidateID] = struct{}{}
		for scoreName, score := range map[string]float64{
			"resource_fit":    candidate.Scores.ResourceFit,
			"slo_headroom":    candidate.Scores.SLOHeadroom,
			"cost_efficiency": candidate.Scores.CostEfficiency,
			"availability":    candidate.Scores.Availability,
			"total":           candidate.Scores.Total,
		} {
			if score < 0 || score > 1 {
				return fmt.Errorf("candidate %s score %s must be between 0 and 1", candidateID, scoreName)
			}
		}
	}
	if recommendation.Status == "FOUND" && strings.TrimSpace(recommendation.SelectedCandidateID) == "" {
		return fmt.Errorf("resource_recommendation.selected_candidate_id is required when status is FOUND")
	}
	return nil
}
...
func summarizeFeedback(flow Flow) *FeedbackSummary {
	...
	if feedback.ErrorCode != "" {
		summary.ErrorCode = feedback.ErrorCode
	}
...
func cloneFlow(flow Flow) Flow {
	...
	if flow.Decision != nil {
		result.Decision = cloneAutomationDecision(flow.Decision)
	}
	if flow.RepairDecision != nil {
		result.RepairDecision = cloneAutomationDecision(flow.RepairDecision)
	}
