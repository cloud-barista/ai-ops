...
func TestAutomationRunAPIRequiresTheTrustedSafeguardEndpoint(t *testing.T) {
	...
	response := performJSONRequest(
		...
		`{"input_type":"natural_language","request":"GPU 배포 계획을 준비해 주세요."}`,
	)
	if response.Code != http.StatusNotFound {
		t.Fatalf("unsafe legacy route: code=%d body=%s", response.Code, response.Body.String())
	}
}
