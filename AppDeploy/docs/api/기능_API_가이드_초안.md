...
4. `POST /api/v1/deployments`에는 발급된 `app_version_id` 또는 Planner가 만든 `manifest`를 보낸다. `target_profile_id`는 선택적 hint이며, 생략하면 App Deployer가 등록 Target의 VM readiness와 Manifest 자원 요구사항을 검사해 적합한 Target을 선택한다. 응답의 `target_profile_id`가 실제 선택 결과다.
...
### Deployment Manifest 입력

웹/CLI의 기존 ID 조합 요청은 서버가 다음 형태의 Manifest로 변환해 Deployment 응답에 포함한다. Planner Manifest에는 App 요구사항과 자원 envelope를 넣고 Target hint는 선택적으로 넣는다. VM readiness 검사와 실제 Target 선택, 실행 방식 결정은 App Deployer가 담당한다. AppDeploy는 Manifest에 Secret, SSH key, VM credential 원문을 넣지 않는다.
...
Planner 연동 전에도 동일한 `POST /api/v1/deployments`에 `manifest`만 보내 배포를 요청할 수 있다. 기존 웹 화면은 하위 호환을 위해 ID 조합을 계속 보내며, 두 형식을 동시에 보낼 경우 값이 일치해야 한다.

### Target Profile 등록 삭제

`DELETE /api/v1/target-profiles/{target_profile_id}`는 Target Profile 등록 정보만 삭제한다. 해당 Profile을 참조하는 Deployment가 없거나 모두 `STOPPED`일 때 200을 반환하고, STOPPED가 아닌 참조가 있으면 `TARGET_PROFILE_INVALID`/409를 반환한다. 없는 ID는 `NOT_FOUND`/404이다.

공통 `ProfileDeleteResponse`는 `request_id`, `profile_type`, `profile_id`, `name`, `deleted=true`, `inventory_deleted`, `deleted_at`을 항상 반환한다. Target 삭제는 현재 Resource Inventory snapshot이 실제 제거됐는지를 반환한다.
