...
제출 패키지 정리는 `deliverables/release/1차년도_제출_패키지_체크리스트.md`, 시험 증적 기준은 `deliverables/evidence/증적_패키지_가이드.md`, 외부 연동 책임 경계는 `deliverables/interface/external/외부_연동_경계_정리.md`를 따른다.
...
| 시험 로그 및 결과 | JSON/Markdown/Text | deliverables/evidence/results/ |
...
| 산출물 | 위치 | 역할 |
| --- | --- | --- |
| 외부 제공 인터페이스 명세서 | `deliverables/interface/spec/KHU_AI_App_Deployer_외부제공인터페이스_명세서.md` | 외부 호출 범위, 요청·응답, 상태·에러, 책임 경계 정의 |
| 상태 및 에러 코드 정의 | `agent_md/00_scope_common_contract.md`, `contracts/openapi/openapi.yaml` | Deployment 상태값과 표준 error_code 정리 |
| 외부 연동 책임 경계 | `deliverables/interface/external/외부_연동_경계_정리.md` | ETRI/Innogrid/Bespin/API Gateway 책임 분리 |
| 인터페이스 예제 | `deliverables/interface/examples/requests`, `deliverables/interface/examples/responses` | 외부 연동 주체가 바로 호출 가능한 JSON 예제 |
